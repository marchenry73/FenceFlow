...
import com.fenceestimator.app.data.TakeoffLineMerge
 ...
 *  - which runs get new line items (TakeoffRefresher: never a teardown run);
 *  - what a regenerate does to the lines already on a run (TakeoffLineMerge.plan,
 *    as Repository.replaceAutoGeneratedLineItemsForRun applies it);
 *  - which rows no regenerate touches (role NONE, and rows with no run);
 *  - the order the totals sum in (EstimateLineItemDao: sort_order, then sync_id).
 *
 * The regenerate is the phone's own TakeoffLineMerge, CALLED here rather than
 * restated, so the fixtures pin the rule the app runs: a line somebody edited
 * is kept exactly as it is and stands in for the built line it corrects, the
 * generated lines give way, supplier quotes carry by role, and a typed price
 * is not copied onto other lines of its role. price-job ports it
 * (pricing/line-items.ts mergeTakeoff, load.ts buildCommitPlan). Until
 * 2026-09-22 this file restated the office's older rule instead -- replace
 * edited lines, carry their prices by role -- so the fixtures agreed with the
 * office while the phone kept edited lines, and a quantity typed on the phone
 * reverted whenever the office saved a rate with every fixture green.
 */
object PricingRunner {
    ...
    fun price(input: PricingInput): PricingOutput {
        ...
        val takeoffItems = mutableListOf<EstimateLineItem>()
        ...
        runs.forEach { run ->
            ...
            // This run's lines as the regenerate reads them
            // (EstimateLineItemDao.getGeneratedForRun: sortOrder, then syncId).
            // The order decides which of two quotes on one role carries, and
            // the order the kept lines come out in.
            ...
            // generated lines go. A line somebody typed a number into is
            // theirs and stays (TakeoffRefresher.refreshRun merges a teardown
            // run with nothing built).
            if (run.isTeardown) {
                takeoffItems += TakeoffLineMerge.plan(existingForRun, emptyList()).keptEdited
                return@forEach
            }
            ...
            // The regenerate, by the phone's own rule: the lines to write,
            // then the lines somebody edited, exactly as they are.
            val plan = TakeoffLineMerge.plan(existingForRun, built.items)
            plan.insert.filter { it.unitPrice <= 0.0 }.forEach { zeroPricedIds += it.syncId }
            takeoffItems += plan.insert
            takeoffItems += plan.keptEdited
        }

        // What is on the job after the regenerate, and therefore what the
        // totals see: every run's takeoff lines above, plus the rows no
        // regenerate touches -- hand-typed extras (role NONE) and rows with
        // no run.
        ...
        val itemsForTotals = (takeoffItems + survivors)
            .sortedWith(compareBy({ it.sortOrder }, { it.syncId }))
