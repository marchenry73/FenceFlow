...
fun EstimateScreen(jobId: Long, onBack: () -> Unit, onOpenSupplierPrices: (Long) -> Unit = {}) {
    ...
    val warnings = remember(currentJob, runs, lineItems, totals, changeOrders) {
        EstimateEngine.estimateWarnings(currentJob, runs, lineItems, totals, changeOrders)
    }
    ...
    editingItem?.let { item ->
        ...
        EditLineItemDialog(
            ...
            // DELETE_RECORDS; without it, only a line the takeoff generated
            // on a run (supabase_r6_price_stability.sql's carve-out --
            // replacing takeoff lines is editing the estimate). Offering
...
/** A line the takeoff made and nobody has changed by hand, on a run. */
private fun lineIsTakeoffGenerated(item: EstimateLineItem): Boolean =
    item.isAutoGenerated && item.role != MaterialRole.NONE && item.fenceRunId != null

/** A takeoff line somebody changed by hand, which regenerates now leave alone. */
private fun lineIsHandEditedTakeoff(item: EstimateLineItem): Boolean =
    !item.isAutoGenerated && item.role != MaterialRole.NONE && item.fenceRunId != null
    ...
    Card(onClick = onClick, modifier = Modifier.fillMaxWidth()) {
        Row(
            ...
            Column(Modifier.weight(1f)) {
                ...
                // A takeoff line somebody changed keeps its number through
                // every regenerate now, so it has to say so -- otherwise a
                // line that stops following the drawing looks like a bug.
                if (!item.isAutoGenerated && item.role != MaterialRole.NONE) {
                    Text(
                        stringResource(R.string.est_line_edited),
...
private fun EditLineItemDialog(
    ...
    // The price this line is actually charging, which is the supplier's figure
    // when there is one. Showing unitPrice meant the box opened with a number
