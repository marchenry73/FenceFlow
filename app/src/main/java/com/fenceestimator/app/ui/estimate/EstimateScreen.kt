...
fun EstimateScreen(jobId: Long, onBack: () -> Unit) {
    ...
    val warnings = remember(currentJob, runs, lineItems) {
        EstimateEngine.estimateWarnings(currentJob, runs, lineItems, viewModel.totals())
    }
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (currentJob.calibrationPixelsPerFoot == null) {
                item {
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Text(
                            "Draw and calibrate your fence runs on the Survey screen before generating quantities.",
            ...
            item { TotalsCard(viewModel, currency) }
...
private fun RunSection(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp)) {
            ...
            // Always tappable: if something's missing the button now says what,
            // which is far more useful than a greyed-out control with no reason.
...
@Composable
private fun TotalsCard(viewModel: EstimateViewModel, currency: NumberFormat) {
