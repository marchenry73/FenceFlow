...
data class CloudJob(
    ...
    @SerialName("gate_rate_per_ft") val gateRatePerFt: Double = 0.0,
    @SerialName("trash_haul_fee") val trashHaulFee: Double = 0.0,
    @SerialName("pricing_tier_name") val pricingTierName: String = "",
    @SerialName("tip_amount") val tipAmount: Double = 0.0,
...
internal fun CloudJob.mergeOnto(local: Job): Job = local.copy(
    ...
    laborRatePerFt = laborRatePerFt,
    laborFlatFee = laborFlatFee,
    minimumJobCharge = if (minimumJobCharge > 0.0) minimumJobCharge else local.minimumJobCharge,
    wastePercent = wastePercent,
    blockedReason = blockedReason,
    overrunReason = overrunReason,
    gridExtentFt = gridExtentFt,
    ...
    teardownRatePerFt = teardownRatePerFt,
    teardownFeet = teardownFeet,
    gateRatePerFt = if (gateRatePerFt > 0.0) gateRatePerFt else local.gateRatePerFt,
    trashHaulFee = if (trashHaulFee > 0.0) trashHaulFee else local.trashHaulFee,
    pricingTierName = pricingTierName.ifBlank { local.pricingTierName },
    tipAmount = if (tipAmount > 0.0) tipAmount else local.tipAmount,
