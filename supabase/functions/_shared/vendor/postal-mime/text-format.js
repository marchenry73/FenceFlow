...
// ---- FenceFlow local patch (the only change to this vendored copy) --------
// The regex chain in htmlToText() below is quadratic or worse on unclosed
// constructs: every "<script>" without "</script>", every "<!--" without
// "-->", every "<" without a ">" and every lone CR or U+2028 before a
// "</body>" makes one regex scan to the end of the string. Measured on
// 3.0.0: 24 KB of "<" took 1.6 s, 64 KB of "<script>" 1.3 s, and 30 KB of
// "<a href=x " never finished. PostalMime.parse() calls htmlToText() itself
// for a multipart/mixed with a text/plain part and a text/html part, so one
// crafted email could never be opened. This pass runs first, in linear time,
// and hands the chain input in which every construct is closed:
//  - CR, CRLF, U+2028 and U+2029 become LF (the chain's "." then matches
//    every character, so no ".*$" can fail and retry);
//  - comments are removed; an unclosed one removes the rest, as a browser
//    does;
//  - script and style elements are removed; an unclosed one removes the rest;
//  - a "<" whose ">" does not come before the next "<" becomes a space, so
//    tag spans never overlap and each "<...[^>]*>" regex is linear.
// tests/mail-meta.test.mjs times the hostile inputs above through
// PostalMime.parse(); re-apply this on upgrade unless upstream has fixed it.
function fenceflowDefuseHtml(str) {
    ...
    for (;;) {
        ...
        out += str.slice(pos, m.index) + ' ';
        let end;
        if (m[0] === '<!--') {
            const close = str.indexOf('-->', m.index + 4);
            end = close < 0 ? str.length : close + 3;
        } else {
            const closeRe = m[1].toLowerCase() === 'script' ? /<\/script\b[^<>]*>/gi : /<\/style\b[^<>]*>/gi;
            closeRe.lastIndex = m.index + m[0].length;
            const close = closeRe.exec(str);
            end = close ? close.index + close[0].length : str.length;
        }
        pos = end;
    }
    // Built from slices rather than a character array: a 10 MB part would
    // otherwise cost an array of ten million strings.
    let result = '';
    let from = 0;
    let pending = -1;
    const stray = at => {
        result += out.slice(from, at) + ' ';
        from = at + 1;
    };
    for (let i = 0; i < out.length; i++) {
        const c = out.charCodeAt(i);
        if (c === 60) {
            if (pending >= 0) stray(pending);
            pending = i;
        } else if (c === 62) {
            pending = -1;
        }
...
// ---- end of FenceFlow local patch ----------------------------------------

export function htmlToText(str) {
    str = fenceflowDefuseHtml(str)
