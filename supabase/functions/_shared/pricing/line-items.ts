...
/** An EstimateLineItem plus the catalog category it came from, which the office groups by. */
export interface BuiltLineItem extends EstimateLineItem {
  category: string;
}
...
export interface BuiltItems {
  ...
  /** The same items by line sync id, which is how the contract reports them. */
  zeroPricedSyncIds: string[];
}
...
export function buildLineItems(
  ...
  const zeroPricedSyncIds: string[] = [];
  let order = 0;
  ...
  for (const entry of mergedEntries) {
    ...
    if (chosen.unitPrice <= 0.0) zeroPricedSyncIds.push(syncId);
  ...
  return {
    ...
    zeroPricedSyncIds: distinct(zeroPricedSyncIds),
  };
}
 ...
 */
export function carryOverPrices(built: readonly BuiltLineItem[], existing: readonly EstimateLineItem[]): BuiltLineItem[] {
