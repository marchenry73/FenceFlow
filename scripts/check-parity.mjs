...
const JDK_HOME = process.platform === "win32"
 ...
 */
function suite(label, command, args, extraEnv = {}) {
  const started = Date.now();
  const result = spawnSync(command, args, {
    cwd: REPO_ROOT,
    encoding: "utf8",
    // gradlew.bat and npx.cmd are batch files, which Node will not exec
...
const kotlin = suite(
  ...
  ["testDebugUnitTest", "--tests", '"*Parity*"', "--console=plain", "-q"],
  {
    JAVA_HOME: JDK_HOME,
...
const typescript = suite(
  ...
  ["-y", "tsx", "supabase/functions/_shared/pricing/parity.ts"],
);
